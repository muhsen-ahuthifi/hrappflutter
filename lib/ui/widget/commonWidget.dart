...
cardFlatBoxDecoration() => BoxDecoration(
      color: SmartAppTheme.cardBackground,
      //   backgroundBlendMode: BlendMode.dstOver,
      borderRadius: BorderRadius.only(
          topRight: Radius.circular(8.0),
...
smartErrorToast(BuildContext context, String title, String msg) {
  // => Flushbar(
  //     title: title,
  //     message: msg,
...
// https://stackoverflow.com/questions/55382651/use-snackbar-with-cupertinopagescaffold
  ...
  //          .showSnackBar(SnackBar(content: Text('$title:$msg')));
           showCupertinoSnackBar(context:context,
           backgroundColor: CupertinoColors.systemRed,

           message: '$title:$msg');

// showCupertinoModalPopup(context: context, builder: (context) => SnackBar(content: Text('$title:$msg')),);
}
smartSuccessToast(BuildContext context, String title, String msg) {
  //  Flushbar(
  //     title: title,
  //     message: msg,
  //     duration: Duration(seconds: 3),
...
smartConfirmDialog(
    BuildContext context, String title, String msg,void Function() okFn) {
  showDialog(
    context: context,
...
class DialogOptions {
  ...
  bool checked;
  bool reject;
  DialogOptions({required this.key,required this.label, required this.checked,required this.reject});
}

void smartOptionsDialog(BuildContext context, String title,
    List<DialogOptions> options, Function(String key) callbackFn) {
  showDialog(
    context: context,
    builder: (context) => SimpleDialog(
      ...
      children: options
        ...
        if (value != null) {callbackFn(value)}
      });
}

smartStateDialog(BuildContext context, String title,
    List<DialogOptions> options, final List<Widget> children) {
...
smartOptionsBottomSheet(BuildContext context, String title,
    List<DialogOptions> options, Function(String key) callbackFn) {
  showModalBottomSheet(
    context: context,
    builder: (context) => ListView(
...
class CurvePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    List<Color> colors = this.angle <= 180
        ? [
            Colors.red,
            Colors.red,
          ]
